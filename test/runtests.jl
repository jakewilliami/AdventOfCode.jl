...
@testset "AdventOfCode.jl" begin
    @testset "AdventOfCode.jl/Multidimensional" begin
        ...
        @testset "AdventOfCode.jl/Multidimensional/Indexing" begin
            ...
            I1 = []
            for i in CartesianIndicesRowWise(M1)
                push!(I1, i)
            end
            I2 = collect(Base.Iterators.flatten((CartesianIndex(row_i, col_i) for
                                                 col_i in 1:length(row)) for
                                                (row_i, row) in enumerate(eachrow(M1))),)
            @test I1 == I2
        end
        ...
        @testset "AdventOfCode.jl/Multidimensional/Directions" begin
            ...
            @test all(
                ∈(ℝ²_cartesian_directions), cartesian_directions(2; include_origin = true)
            )
            ...
            @test all(_direction_in_dims(d; dims = :) for
                      d in cartesian_directions(2) if !_is_cardinal(d))
            for (d1, d2) in (
                (INDEX_RIGHT, INDEX_LEFT),
                (INDEX_UP, INDEX_DOWN),
                (INDEX_NORTH_EAST, INDEX_SOUTH_WEST),
                (INDEX_NORTH_WEST, INDEX_SOUTH_EAST),
            )
        ...
        @testset "AdventOfCode.jl/Multidimensional/Adjacencies" begin
            ...
            indices1, indices2 = adjacent_cartesian_indices(M3, CartesianIndex{2}()),
            cartesian_adjacencies_with_indices(M3, CartesianIndex{2}())
